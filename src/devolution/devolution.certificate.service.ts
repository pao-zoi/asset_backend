...
export class DevolutionCertificate {
  ...
  async htmlContent(assets, person, responsible, cargo?) {
    ...
    try {
      const res = await this.httpService.get(
        `${getConfig().api_personal}api/personal/${responsible.idUser}`).toPromise();
      responsibleFound = res.data
    } catch (error) {
      throw error.response?.data
    }
    const { name:nameResponsible, lastName:lastNameResponsible} = responsibleFound
  ...
  const generateAssetRows = (assets.assetId ? assets.assetId : assets).map(asset => {
